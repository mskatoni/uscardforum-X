...
| Cloudflare盾 | 检测 403/回应失败弹窗并跳转 `/challenge`；关闭时同一菜单行可强制触发 | 只监听 Discourse 弹窗 |
...
自动模块会检测部分 Discourse 失败弹窗，例如 403、回应太旧、不能 reaction 等，然后跳转：

```text
/challenge?redirect={当前页面}
```

开启时菜单显示 `✅ Cloudflare盾`，点击会关闭自动监听。关闭时同一菜单行显示 `强制触发Cloudflare盾`，点击会立即启用模块并跳转 Challenge 页面。

### 自动阅读

`自动阅读` 默认关闭。开启后，打开论坛首页、最新/未读/热门/分类页或帖子页即可开始。
