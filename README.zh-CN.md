...
- 短回复提交前自动补图片 Markdown，避免有效字数不足 4，可在菜单关闭
- 油猴菜单触发当前可见帖子的原生 Discourse Reactions Like 按钮
- 授权测试用自动点赞：开启后在自动阅读滚动时触发当前可见 Like 按钮
...
| 短回复补图 | 有效字数为 1-3 时，提交前自动追加图片 Markdown | Discourse 编辑器提交按钮；默认开启 |
| 点赞可见帖子 | 触发当前可见且未点赞的原生 Discourse Reactions Like 按钮 | 油猴菜单命令，只在当前页面即时执行 |
...
带 `*` 的是 TL3/白金隐藏条件。只有论坛允许读取 `user_actions.json` 时才会显示，避免显示假数据；这些隐藏检查会带重试、退避和页间延迟，避免瞬间打出一串用户动作历史请求。
...
`短回复补图` 默认开启，可在油猴菜单里关闭。回复或发帖内容的有效字数为 1-3 时，脚本会在提交前自动追加图片 Markdown，帮助通过论坛最小长度检查，并短暂高亮文本框让这次改写可见。计数时会忽略引用块，`:sticker:` 形式的表情按 1 个字符计算；如果整条内容只有一个表情，则保持原样不补图。
